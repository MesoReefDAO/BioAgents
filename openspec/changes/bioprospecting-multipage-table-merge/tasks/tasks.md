...
- [ ] 4.2 Mount `ChainPager` in the lightbox header next to the title when `provenance.type === "table"` AND the source's `evidence.tables` includes a chain containing the current fact's table. Fetch the chain via a new helper (e.g. `useTableChain(currentTableId)`) that walks `continuesFromId` on the cached evidence. Use the same `Set<string>` cycle-detection pattern as the prompt walker (defensive).
