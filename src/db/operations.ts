 ...
 *
 * Lookup order:
 *   1. Privy DID (users.user_id)
 *   2. Email — if the DID is new but the email already exists (Privy app
 *      rotation), re-attach the DID to the existing row so whitelist/admin
 *      and conversation history survive.
 *   3. Insert a fresh pending user
 */
